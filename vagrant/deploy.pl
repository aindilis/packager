...
die "Need a name and version\n" unless $conf->{'-n'} and $conf->{'-v'};

ActionDeploy
  (
   Name => $conf->{'-n'}, # i.e. 'relationfactory',
   Version => $conf->{'-v'}, # i.e. '20140521',
  );
...
my $dir = '/var/lib/myfrdcsa/codebases/internal/packager/vagrant/packages/'.$conf->{'-n'}.'-'.$conf->{'-v'};
if (-d $dir) {
  my $command = 'cd '.$dir.' && uxterm +lc -e \'screen -S '.$conf->{'-n'}.
    ' vagrant up; vagrant ssh -c "screen -S user emacs -nw -e shell"\'';
  print $command."\n";
  system $command;
}
